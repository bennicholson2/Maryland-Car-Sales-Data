...
(
  ...
  values_new_cars_sold INT NOT NULL,
  value_used_cars_sold INT NOT NULL,
  month DATE,
  PRIMARY KEY (month)
);
...
(
  ...
  inflation_rate_cpi INT NOT NULL,
  average_weekly_wage INT NOT NULL,
  total_wages_thousands INT NOT NULL,
  year DATE,
  FOREIGN KEY (year) REFERENCES yearly_car_sales(year)
);

CREATE TABLE yearly_socioeconomic_data
(
...
(
  inflation_rate_cpi INT NOT NULL,
  unemployment_rate_not_sa INT NOT NULL,
  unemployment_rate_sa INT NOT NULL,
  month DATE,
